...
opts.load_rl_net = [];
opts.fineTunePath = 'rl/fineTune data';
...
for t=1:params.batchSize:numel(subset)
  ...
  if strcmp(mode, 'train')
      rlStep = [rlStep, gather(res(end).rlStep)];
      classLoss = [classLoss, gather(stats.lossForClassification)];
      mi = [mi, gather(res(end).mi)];
  end
...
if strcmp(mode, 'train')
    rl_folder = sprintf('%s/E_%d', params.fineTunePath, params.epoch);
    if ~exist(rl_folder, 'dir')
        mkdir(rl_folder)
    end
    save(fullfile(rl_folder, 'rlStep.mat'), 'rlStep');
    save(fullfile(rl_folder, 'classLoss.mat'), 'classLoss');
    save(fullfile(rl_folder, 'mi.mat'), 'mi');
end
% Save back to state.
